...
Err:
    ...
    Sub WRPlot(stns As String, sdt As String, edt As String)
        ...
        sql = "SELECT recordedFrom as STNID,year(obsDatetime) as YY,month(obsDatetime) as MM,day(obsDatetime) as DD,hour(obsDatetime) as HH,SUM(IF(describedBy = '112', value, NULL)) AS 'DIR',AVG(IF(describedBy = '111', value, NULL)) AS 'WSPD' FROM (SELECT recordedFrom, describedBy, obsDatetime, obsValue value FROM observationfinal WHERE (RecordedFrom = " & stns & ") AND (describedBy = '111' OR describedBy = '112') and (obsDatetime between '" & sdt & "' and '" & edt & "') ORDER BY recordedFrom, obsDatetime) t GROUP BY STNID, YY, MM, DD, HH;"
        ...
        For k = 0 To maxRows - 1
            For i = 0 To ds.Tables("observationfinal").Columns.Count - 1
                ...
                If i = ds.Tables("observationfinal").Columns.Count - 1 And Int(RegkeyValue("key13")) = 2 Then
                    datval = datval * 10
                End If
...
Err:
    ...
    Sub CPTProducts(st As String, ed As String)
        ...
        For k = 0 To lstvElements.Items.Count - 1
            ...
            If lstvElements.Items(k).SubItems(0).Text = 5 Or lstvElements.Items(k).SubItems(0).Text = 18 Then
                SummaryType = "Sum(obsValue) AS Total"
                SumAvg = "SUM"
            Else
                SummaryType = "Avg(obsValue) AS Mean"
