...
Public Class dataEntryGlobalRoutines
    ...
    Sub shiftEntries(kycode As Integer, frm As Object, tbl As String)
        Select Case kycode
            Case 34 ' Insert
                insertValues(frm, tbl)
                shiftFlags(frm, tbl)
            Case 33 ' Delete
                deleteValues(frm, tbl)
                shiftFlags(frm, tbl)
        End Select
    End Sub

    Sub insertValues(frm As Object, tbl As String)

        Dim ActvCTL, nxtCTL As Control
        Dim kount As Integer
        Dim txt1, txt2 As String

        ActvCTL = frm.ActiveControl
        txt1 = ActvCTL.Text
        'MsgBox(txt1)
        If totalCTLS(kount, tbl) Then
            'MsgBox(kount)
            ActvCTL.Text = ""
            For i = 0 To kount - 2
                nxtCTL = frm.GetNextControl(ActvCTL, True)
                'MsgBox(ActvCTL.Name & " " & nxtCTL.Name)
        ...
        End If
    End Sub
    Sub shiftFlags(frm As Object, tbl As String)
        Dim flagIndexDiff As Integer
        Dim ctls As Control
        Dim txt, flg As String

        If Not totalCTLS(flagIndexDiff, tbl) Then Exit Sub
        For Each ctls In frm.Controls
            If Strings.Left(ctls.Name, 6) = "txtVal" Then
                ...
                FlagValue(frm, ctls, txt, flg)
            End If
        Next
    End Sub
    Sub deleteValues(frm As Object, tbl As String)
        Dim ActvCTL, nxtCTL As Control
        Dim kount As Integer
        ...
        If totalCTLS(kount, tbl) Then
            For i = 0 To kount - 1
                ...
                Do While nxtCTL.Enabled = False
                    nxtCTL = frm.GetNextControl(nxtCTL, True)
                Loop
    ...
    Function totalCTLS(ByRef kount As Integer, tbl As String) As Boolean
        ...
        sql = "select * from " & tbl & ";" 'form_synoptic2_tdcf;"
    ...
    Sub FlagValue(frm As Object, txtCTL As Control, txt As String, flg As String)
        Dim flgCTL As Control

        For Each flgCTL In frm.Controls
