...
Public Class frmUploadToObsFinal
    ...
    Private Sub butOK_Click(sender As Object, e As EventArgs) Handles butOK.Click
        ...
        Try
            ...
            'sql = "Select recordedFrom, describedBy FROM observationinitial WHERE qcStatus=1 And entry_date_time BETWEEN '" & dtfrom & "' AND '" & dtTo & "'
            ...
            '        For j = 0 To LstViewStations.Items.Count - 1
            '            If LstViewStations.Items(j).SubItems(0).Text = .Rows(i).Item("recordedFrom") Then
            '                LstViewStations.Items(j).Checked = True
            '            End If
            '        Next
            '    Next
            ...
            '            If lstViewElements.Items(j).SubItems(0).Text = .Rows(i).Item("describedBy") Then
            '                lstViewElements.Items(j).Checked = True
            '            End If
            '        Next
            '    Next
            'End With
            ...
            sql = "Select recordedFrom as stationId, stationName FROM observationinitial INNER join station On recordedFrom = stationId WHERE qcStatus = 1
                  And entry_date_time BETWEEN '" & dtfrom & "' AND '" & dtTo & "'
                  GROUP BY recordedFrom, stationName;"
            ...
            With ds.Tables("lst")
                For i = 0 To .Rows.Count - 1
                    stn(0) = .Rows(i).Item("stationId") '"0123456789"
                    stn(1) = .Rows(i).Item("stationName")
                    cmbstation.Items.Add(stn(1))
                    itms = New ListViewItem(stn)
                    LstViewStations.Items.Add(itms)
                Next
            ...
            sql = "SELECT describedBy as elementId, elementName FROM observationinitial INNER join obselement on describedBy = elementId WHERE qcStatus = 1
                  And entry_date_time BETWEEN '" & dtfrom & "' AND '" & dtTo & "'
                  GROUP BY describedBy, elementName;"
            ...
            With ds.Tables("lst")
                For i = 0 To .Rows.Count - 1
                    elm(0) = .Rows(i).Item("elementId")
                    elm(1) = .Rows(i).Item("elementName")
                    cmbElement.Items.Add(elm(1))
                    itms = New ListViewItem(elm)
                    lstViewElements.Items.Add(itms)
                Next
    ...
    Private Sub cmdUploadData_Click(sender As Object, e As EventArgs) Handles cmdUploadData.Click
        ...
        ' Definition of other data fields
        Dim qcTypeLog, acquisitionType, dataform, captBy, tmpUnits, precipUnits, cldUnits, visunits, datasrcTzone, entryDatetime As String
        Dim yyyy As Integer
        ...
        Try
            ...
            If chkEntrydate.Checked = True Then
                sql = "SELECT * FROM observationinitial WHERE " & stnelm_selected & " qcStatus=1;"
                'MsgBox(sql)
            End If
            ...
            For n = 0 To maxRows - 1
                ...
                Try
                    ...
                    If Not IsDBNull(ds.Tables("obsInitial").Rows(n).Item("visUnits")) Then visunits = ds.Tables("obsInitial").Rows(n).Item("visUnits")
                    If Not IsDBNull(ds.Tables("obsInitial").Rows(n).Item("dataSourceTimeZone")) Then datasrcTzone = ds.Tables("obsInitial").Rows(n).Item("dataSourceTimeZone")
                    If Not IsDBNull(ds.Tables("obsInitial").Rows(n).Item("entry_date_time")) Then entryDatetime = ds.Tables("obsInitial").Rows(n).Item("entry_date_time")
                    'Generate SQL string for inserting data into observationFinal table

                    entryDatetime = DateAndTime.Year(entryDatetime) & "-" & DateAndTime.Month(entryDatetime) & "-" & DateAndTime.Day(entryDatetime) & " " & DateAndTime.Hour(entryDatetime) & ":" & DateAndTime.Minute(entryDatetime) & ":" & DateAndTime.Second(entryDatetime)
                    'MsgBox(entryDatetime)
                    ...
                    If Not chkUpdateRecs.Checked Then
                        ...
                        rec = stnId & "," & elemCode & "," & obsDate & "," & obsLevel & "," & obsVal & "," & obsFlag & "," & period & "," & qcStatus & "," & qcTypeLog & "," & acquisitionType & "," &
                           dataform & "," & captBy & "," & mark1 & "," & tmpUnits & "," & precipUnits & "," & cldUnits & "," & visunits & "," & datasrcTzone & "," & entryDatetime
                    ...
                    Else
                        ...
                        'If dataform = "\N" Then dataform = "NULL"
                        If captBy = "\N" Then captBy = String.Empty
                        If tmpUnits = "\N" Then tmpUnits = String.Empty
                        If precipUnits = "\N" Then precipUnits = String.Empty
                        If cldUnits = "\N" Then cldUnits = String.Empty
                        If visunits = "\N" Then visunits = String.Empty
                        If datasrcTzone = "\N" Then datasrcTzone = String.Empty
                        ...
                        "dataForm,capturedBy,mark,temperatureUnits,precipitationUnits,cloudHeightUnits,visUnits,dataSourceTimeZone,entry_date_time) " &
                ...
                Catch et As Exception
                    'MsgBox(et.Message & " at records transfer")
                    Continue For
                End Try
        Next n
        ...
        Catch ex As Exception
            MsgBox(ex.Message & " at cmdUploadData")
            lblTableRecords.ForeColor = Color.Red
            lblTableRecords.Text = "Data transfer failed !"
            conn.Close()
            FileClose(102)
