...
ident_var <- function (data,variables) {
  ...
  if(!(station_label %in% names(merged))){
    for (label in c("Station","station","Station_id","station_id","ID","ID.", "Name", "name","StationName","StationID", "recordedFrom")){
  ...
  if(!(date_time_label %in% names(merged))) {
    for (label in c("Date time","date time","date_time", "obsDatetime")){
      if (label %in% names(data)){
  ...
  if(!(rain_label %in% names(merged))) {
    for (label in c("Rain","rain","Observed","observed","obsValue","obs_value", "Prec","prec","Precipitation","Precipitation (mm)","precipitation","precipitation (mm)", "RR","prcp")){
      if (label %in% names(data)){
